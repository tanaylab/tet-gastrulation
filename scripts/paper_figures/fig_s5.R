...
host_vs_dko_age = function(genotype,mat_nm,plot_pdf = F) {
  ...
  plot(rank_to_time$developmental_time[chimera_age$best_rank_ko[f]],
       rank_to_time$developmental_time[chimera_age$best_rank_host[f]],pch = 19,
...
plot_dko_chimera_dotplots = function(dko_genotype,plot_pdf = T) {
  ...
  stat_comparison = data.frame(group1 = c(rep(toupper(dko_genotype),length(highlighted_colors)),rep("Host",length(highlighted_colors)),rep(toupper(dko_genotype),length(highlighted_colors))),
  ...
  for (ct_col in highlighted_colors) {
    ...
    df_plot_points = data.frame(genotype = factor(x = c(rep(toupper(dko_genotype),nrow(tko_freq_n)),rep("Host",nrow(host_freq_n)),rep("WT",nrow(wt_freq_n))),levels = c(toupper(dko_genotype),"Host","WT")),
...
dko_dotplots_new = function(gt = "dko12",plot_pdf = T) {
  ...
  stat_comparison = data.frame(group1 = c(rep("DKO",length(highlighted_colors)),rep("Host",length(highlighted_colors)),rep("DKO",length(highlighted_colors))),
                               group2 = c(rep("Host",length(highlighted_colors)),rep("WT",length(highlighted_colors)),rep("WT",length(highlighted_colors))),
  ...
  for (ct_col in highlighted_colors) {
    ...
    df_plot_points = data.frame(genotype = factor(x = c(rep("DKO",nrow(ko_freq_n)),rep("Host",nrow(host_freq_n)),rep("WT",nrow(wt_freq_n))),levels = c("DKO","Host","WT")),
