...
heatmap_cell_types_query_vs_wt <- function(genotype, plot_pdf = F) {
    ...
    pheatmap::pheatmap(query_wt_cor,
        color = shades, cluster_rows = F, cluster_cols = F, annotation_row = ct_annotation, annotation_col = ct_annotation, annotation_colors = annotation_colors,
...
scatter_plots_dko <- function(plot_pdf = F) {
    ...
    clone_assay_labels <- c(
        "Ctrl1 Chimera" = "Control 1",
    ...
    df_text <- data.frame(
        mean_lfp = paste0("av. lfc = ", round(colMeans(abs(lfp_type[genes_all, ])), 2)),
    ...
    p2 <- ggplot(data = df_hm, aes(y = cell_type, x = mean_dist, fill = Genotype)) +
        ...
        xlab("Mean absolute log2 fold change") +
        ylab("") +
...
cmp_egc_per_type_without_tetraploid <- function(included_cts, n_downsample, seed_downsample = 123456, downsample_umis = NULL) {
    ...
    md_all <- bind_rows(
        mat_chim@cell_metadata[colnames(mat_chim@mat), ],
