...
number_of_cells_per_embryo <- function(plot_pdf = F) {
    ...
    all_embryos <- bind_rows(
        as.data.frame(n1, stringsAsFactors = F),
    ...
    transcriptional_time_all <- bind_rows(
        rename(mat1_time[, c("embryo", "best_query")], "transcriptional_rank" = "best_query"),
...
cell_type_comparisons_s2 <- function() {
    ...
    tko_tetra <- tko_bulk_comparison(
        mat_id = "tko_tetra_wt10",
    ...
    ctrl_tetra <- tko_bulk_comparison(
        mat_id = "control_tetra_all_wt10",
        included_cell_types = c(
            "Epiblast", "Early nascent mesoderm", "Amnion/Chorion", "ExE mesoderm",
            "Primitive streak"
        ),
...
tko_tetraploid_vs_wt_time_distribution <- function(plot_pdf = F) {
    ...
    df_plot <- data.frame(
        age = c(wt_age$developmental_time[query_time_dist_tko$query_time_match], wt_age$developmental_time[atlas_time_f]),
...
plot_tko_germline_barplot_cell_type_composition <- function() {
    ...
    filtered_vs_ct <- table(
        factor(
            x = mat@cell_metadata[filtered_cls, "embryo"],
            levels = tko_germline_embryos
        ),
        factor(
            x = query_cls_col[filtered_cls],
...
plot_tko_germline_nascent_mesosderm_vs_exe_meso_fraction <- function() {
    ...
    tko_germline_embryo_vs_cell_type <- compute_two_way_table(
        values_row = mat_tko_germline@cell_metadata[tko_germline_cells, "embryo"],
    ...
    tko_tetra_embryo_vs_cell_type <- compute_two_way_table(
        values_row = mat_tko_tetra@cell_metadata[tko_tetra_cells, "embryo"],
    ...
    wt_embryo_vs_cell_type <- compute_two_way_table(
        values_row = mat_wt@cell_metadata[names(mc_wt@mc), "transcriptional_rank"],
    ...
    plot(
        x = wt_exe_meso, y = wt_emb_meso, pch = 19, cex = 4, col = "gray30", xlim = c(0, 0.85), ylim = c(0, 0.45),
    ...
    plot(
        x = wt_endo, y = wt_ecto, pch = 19, cex = 4, col = "gray30", xlim = c(0, 0.125), ylim = c(0, 0.6),
