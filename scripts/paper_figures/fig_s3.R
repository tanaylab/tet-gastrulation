...
cell_type_comparisons_s3 = function() {
  ...
  tko_chim = tko_bulk_comparison_s3(mat_id = "tko_chim_wt10",
  ...
  ctrl_chim = tko_bulk_comparison_s3(mat_id = "tko_chim_wt10",
                                     included_cell_types = c("Epiblast","Early nascent mesoderm","Amnion/Chorion","ExE mesoderm",
                                                             "Primitive streak"),
...
tko_bulk_comparison_s3 = function(mat_id,included_cell_types,genotype,plot_pdf = F,xlab_plot = "",tag_fn = "tko_4N") {
  ...
  for (ct in included_cell_types) {
    ...
    p = ggplot(df_plot,aes(y = expression_wt,x = expression_tko)) + 
      ...
      geom_abline(slope = 1,intercept = 1,color= "gray",linetype = "dashed") +
      geom_point(color = gene_color,size =1) + ylab("WT") + xlab(xlab_plot) +
...
fig_s3_ctrl_host_frequency_plots = function() {
  ...
  s3_chimera_plot_frequency_group_of_cell_types(highlighted_colors = mc_wt@color_key$color[2:6],main_tag = paste0(fn_prefix,"Ectoderm"),ctrl_color = ctrl_color,host_color = host_color,
                                                plot_pdf = plot_pdf,plot_wt = plot_wt,plot_host = plot_host,plot_ctrl = plot_ctrl)
  s3_chimera_plot_frequency_group_of_cell_types(highlighted_colors = mc_wt@color_key$color[26:27],main_tag = paste0(fn_prefix,"Endoderm"),ctrl_color = ctrl_color,host_color = host_color,
                                                plot_pdf = plot_pdf,plot_wt = plot_wt,plot_host = plot_host,plot_ctrl = plot_ctrl)
  s3_chimera_plot_frequency_group_of_cell_types(highlighted_colors = mc_wt@color_key$color[c(8,12,13,14,15,16)],
                                                main_tag = paste0(fn_prefix,"Embryonic mesoderm"),ctrl_color = ctrl_color,host_color = host_color,
                                                plot_pdf = plot_pdf,plot_wt = plot_wt,plot_host = plot_host,plot_ctrl = plot_ctrl)
  s3_chimera_plot_frequency_group_of_cell_types(highlighted_colors = mc_wt@color_key$color[c(17,18,19)],
                                                main_tag = paste0(fn_prefix,"Extraembryonic mesoderm"),ctrl_color = ctrl_color,host_color = host_color,
                                                plot_pdf = plot_pdf,plot_wt = plot_wt,plot_host = plot_host,plot_ctrl = plot_ctrl)
  s3_chimera_plot_frequency_group_of_cell_types(highlighted_colors = mc_wt@color_key$color[c(21:23)],
...
fig_s3_ctrl_vs_host_timing = function() {
  ...
  p = ggplot(data = df_plot,aes(x = best_rank_host,y = best_rank_control)) +
    ...
    geom_point(size = 4) + xlab(expression(Host~E[t])) + ylab(expression(Ctrl~E[t])) +
    theme(axis.text = element_text(size = 20),axis.title = element_text(size = 20)) +
    xlim(7,8.1) + ylim(7,8.1)
...
dosage_effect_tko_in_chimeras_dotplots = function(plot_pdf = F) {
  ...
  embryos_f = intersect(unique(mat@cell_metadata[c(host_cells,control_cells),"embryo"]),
  ...
  ko_cell_type_fraction = table(factor(x = mat@cell_metadata[tko_cells,"embryo"],levels = embryos_f),
                                factor(cmp_annot$query_cls_col[tko_cells],levels = mc_wt@color_key$color[1:27]))
  host_cell_type_fraction = table(factor(x = mat@cell_metadata[host_cells,"embryo"],levels = embryos_f),
                                  factor(cmp_annot$query_cls_col[host_cells],levels = mc_wt@color_key$color[1:27]))
  control_cell_type_fraction = table(factor(x = mat@cell_metadata[control_cells,"embryo"],levels = embryos_f),
  ...
  tko_low_to_high_p_values = sapply(all_groups,function(ct_group) {
    ...
    p_val = wilcox.test(x = ko_group_fraction[chimeras_with_high_tko_content,ct_group],
  ...
  host_control_low_to_high_p_values = sapply(all_groups,function(ct_group) {
    ...
    p_val = wilcox.test(x = host_control_group_fraction[chimeras_with_high_tko_content,ct_group],
  ...
  stat_comparison = data.frame(group1 = c(rep("TKO Low",length(all_groups)),rep("Host/Control Low",length(all_groups))),
  ...
  for (ct_group in all_groups) {
    ...
    df_plot_points = data.frame(genotype = factor(x = c(rep("TKO",length(embryos_f)),
                                                        rep("Host/Control",length(embryos_f))),levels = c("TKO","Host/Control")),
                                genotype_dosage = factor(x = c(rep("TKO Low",length(chimeras_with_low_tko_content)),rep("TKO High",length(chimeras_with_high_tko_content)),
                                ...
                                freq = c(ko_group_fraction[chimeras_with_low_tko_content,ct_group],
...
fig_s3_chimera_dotplots_new_lineages = function(plot_pdf = T) {
  ...
  stat_comparison = data.frame(group1 = c(rep("TKO",length(highlighted_colors)),rep("Host/Control",length(highlighted_colors)),rep("TKO",length(highlighted_colors))),
  ...
  for (ct_col in highlighted_colors) {
    ...
    df_plot_points = data.frame(genotype = factor(x = c(rep("TKO",nrow(tko_freq_n)),rep("Host/Control",nrow(host_freq_n)),rep("WT",nrow(wt_freq_n))),levels = c("TKO","Host/Control","WT")),
...
chimera_dotplot_frequencies_lineages = function() {
  ...
  wt10_emb_vs_ct = table(mat_chim@cell_metadata[wt10_cls,"transcriptional_rank"],
                         ct_to_lineage[mc_wt@colors[mc_wt@mc[wt10_cls]]])
  wt10_emb_vs_ct = wt10_emb_vs_ct[t_f,]
  ko_emb_vs_ct = table(mat_chim@cell_metadata[ko_query_cls_f,"embryo"],
                       ct_to_lineage[ko_query_cls_col[ko_query_cls_f]])
  ko_emb_vs_ct = ko_emb_vs_ct[embryos[f_t_q],]
  host_emb_vs_ct = table(mat_chim@cell_metadata[host_query_cls_f,"embryo"],
