...
plot_epiblast_gene_expression_vs_time <- function(genes_f = c("Dppa4", "Gdf3"), plot_pdf = F, fig_dir = "figs/paper_figs/fig3/epiblast_expression_vs_time") {
    ...
    for (gene in genes_f) {
        ...
        plot(
            x = age_group_time[as.numeric(colnames(le_t_wt)[(roll_width + 1):(ncol(le_t_wt) - roll_width)])], y = freq_median, type = "l", lwd = 2, ylim = ylim_plot,
            ylab = "Log2 Expression", xlab = "Developmental Time",
            main = gene, cex.main = 3
        )
        polygon(
            x = c(age_group_time[as.numeric(colnames(le_t_wt)[(roll_width + 1):(ncol(le_t_wt) - roll_width)])], rev(age_group_time[as.numeric(colnames(le_t_wt)[(roll_width + 1):(ncol(le_t_wt) - roll_width)])])),
            y = c(upper_freq, rev(lower_freq)), col = "gray80", border = NA
        )
        lines(
            x = age_group_time[as.numeric(colnames(le_t_wt)[(roll_width + 1):(ncol(le_t_wt) - roll_width)])],
...
cmp_single_cell_score <- function(feat_genes, ct_col) {
    ...
    df_score <- data.frame(
        clone = c(tko_chim@cell_metadata[query_chim_cls, "clone_type"], tko_tetra@cell_metadata[tko_tetra_cls, "clone_type"], ctrl_tetra@cell_metadata[ctrl_tetra_cls, "clone_type"]),
    ...
    df_score_wt <- data.frame(
        clone = rep("WT", length(multi_panel_wt)),
...
compute_mean_and_sd_of_expression <- function(df_score_all) {
    ...
    df_out <- data.frame(
        multi_panel = names(mean_expression),
...
if (0) {
    plot_epiblast_score_distribution_per_genotype_and_experiment <- function(plot_pdf = F) {
        ...
        df_wt_score <- data.frame(
            clone_assay = "WT", experimental_assay = "WT", cell_type = "WT", clone_type = "WT", epiblast_score = wt_score, multi_panel = experiment_genotype_wt,
    ...
    plot_nascent_mesoderm_score_distribution_per_genotype_and_experiment <- function(plot_pdf = F) {
        ...
        df_wt_score <- data.frame(
            clone_assay = "WT", experimental_assay = "WT", cell_type = "WT", clone_type = "WT", nm_score = wt_score, multi_panel = experiment_genotype_wt,
            sc_name = names(wt_score), stringsAsFactors = F
        )
    ...
    old_epiblast_gene_expression_vs_age_group <- function(plot_pdf = F) {
        ...
        for (gene in genes_f) {
            ...
            plot(
                x = age_group_time[as.numeric(colnames(le_t_wt))], y = le_t_wt[gene, ], type = "l",
                ylim = c(min(c(le_t_chim[gene, ], le_t_tetra[gene, ], le_t_wt[gene, ])), max(c(le_t_chim[gene, ], le_t_tetra[gene, ], le_t_wt[gene, ]))),
    ...
    cmp_epiblast_score <- function() {
        ...
        df_clone <- data.frame(
            clone_assay = c("TKO26 Chimera", "TKO26 Tetraploid", "TKO29 Chimera", "TKO29 Tetraploid", "TKO23 Chimera", "Host Chimera", "Ctrl1 Chimera", "Ctrl1 Tetraploid", "Ctrl2 Chimera", "Ctrl2 Tetraploid"),
        ...
        for (i in 1:nrow(df_clone)) {
            ...
            df_tmp <- data.frame(
                clone_assay = rep(tag, length(query_score)),
    ...
    plot_epiblast_score_distribution_per_clone_and_experiment <- function(plot_pdf = F) {
        ...
        df_wt_score <- data.frame(
            clone_assay = "WT", experimental_assay = "WT", cell_type = "WT", clone_type = "WT", epiblast_score = wt_score, multi_panel = clone_assay_wt,
            sc_name = names(wt_score), stringsAsFactors = F
        )
        ...
        p <- ggplot(df_score_all, aes(x = epiblast_score, color = cell_type)) +
            ...
            facet_wrap(multi_panel ~ ., nrow = 2) +
            geom_vline(xintercept = top_value, linetype = "dashed", col = "gray60") +
            ggtitle("Epiblast score distribution among epiblast single cells") +
            scale_color_manual() +
            scale_color_manual(values = cell_type_cols) +
            geom_text(data = df_text, mapping = aes(x = x, y = y, label = n_cls), col = "black", hjust = 0)
    ...
    plot_nascent_mesoderm_score_distribution_per_clone_and_experiment <- function(plot_pdf = F) {
        ...
        df_wt_score <- data.frame(
            clone_assay = "WT", experimental_assay = "WT", cell_type = "WT", clone_type = "WT", nm_score = wt_score, multi_panel = clone_assay_wt,
            sc_name = names(wt_score), stringsAsFactors = F
        )
        ...
        p <- ggplot(df_score_all, aes(x = nm_score, color = cell_type)) +
            ...
            facet_wrap(multi_panel ~ ., nrow = 2) +
            geom_vline(xintercept = top_value, linetype = "dashed", col = "gray60") +
            ggtitle("Nascent mesoderm score distribution among nascent mesoderm single cells") +
            scale_color_manual() +
            scale_color_manual(values = cell_type_cols) +
            geom_text(data = df_text, mapping = aes(x = x, y = y, label = n_cls), col = "black", hjust = 0)
    ...
    plot_nascent_mesoderm_score_single_genes <- function(plot_pdf = T) {
        ...
        for (g in all_genes) {
            print(g)
            mc_gg_chim_tetra_without_control(
                genes1 = early_nascent_mesoderm_genes(),
                genes2 = g,
                xlab = "Nascent mesoderm score",
                ylab = g,
                fig_dir = fig_dir,
    ...
    nascent_mesoderm_differential_expression <- function() {
        ...
        p <- ggplot(df_score_all, aes(x = nm_score, color = cell_type)) +
            ...
            facet_wrap(multi_panel ~ ., nrow = 2) +
            geom_vline(xintercept = top_value, linetype = "dashed", col = "gray60") +
            ggtitle("Nascent mesoderm score distribution among epiblast single cells") +
            scale_color_manual() +
            scale_color_manual(values = cell_type_cols) +
            geom_text(data = df_text, mapping = aes(x = x, y = y, label = n_cls), col = "black", hjust = 0)
        ...
        all_genes <- c(
            "Hand1", "Msx1", "Msx2", "Tbx3", "Foxc1", "Foxc2", "Tbx6", "Dll1", "Dll3", "Twist1", "Gata4", "Gata6", "Lhx1", "Snai1", "Lefty2", "Eomes", "Tdgf1",
            "Aldh1a2"
        )
        ...
        for (g in all_genes) {
            print(g)
            mc_gg_chim_tetra_with_control(
                genes1 = epiblast_genes(), genes2 = g, xlab = "Epiblast score",
