...
fig1_frequencies_per_germ_layer <- function(plot_pdf = F) {
    ...
    plot_frequency_group_of_cell_types(
        highlighted_colors = mc_wt@color_key$color[2:6], main_tag = paste0(fn_prefix, "Ectoderm"), ko_color = ko_color, control_color = control_color,
        plot_pdf = plot_pdf, plot_wt = plot_wt, plot_control = plot_control, plot_ko = plot_ko
    )
    plot_frequency_group_of_cell_types(
        highlighted_colors = mc_wt@color_key$color[26:27], main_tag = paste0(fn_prefix, "Endoderm"), ko_color = ko_color, control_color = control_color,
        plot_pdf = plot_pdf, plot_wt = plot_wt, plot_control = plot_control, plot_ko = plot_ko
    )
    plot_frequency_group_of_cell_types(
...
atlas_projection_control_tetraploid <- function(plot_pdf = F) {
    ...
    fn <- "figs/paper_figs/fig1/atlas_projection_control_tetraploid_all_embryos_new_2.png"
    w <- 1000
    h <- 1000
    if (plot_pdf) {
        fn <- gsub(pattern = ".png", replacement = ".pdf", x = fn)
...
barplot_tetraploid_ct_frequency <- function(mat_nm, plot_pdf = F) {
    ...
    modified_cell_type_levels <- c(
        mc_wt@color_key$group[c(1, 7, 9, 10, 11, 20, 24, 25)], c("space1"),
    ...
    modified_colors <- c(
        mc_wt@color_key$color[c(1, 7, 9, 10, 11, 20, 24, 25)], c("white"),
...
var_vs_mean_tko_ctrl <- function() {
    ...
    plot(
        x = wt_mean[(rollwidth + 1):(length(wt_mean) - rollwidth)], y = wt_var_roll_mean, type = "l", lwd = 2,
        ylim = c(0, 0.16), xlim = c(6.85, 7.92),
        xlab = "Mean Et", ylab = "Variance Et"
    )
    polygon(
        x = c(wt_mean[(rollwidth + 1):(length(wt_mean) - rollwidth)], rev(wt_mean[(rollwidth + 1):(length(wt_mean) - rollwidth)])),
