...
trajectories_epigenetic_factors = function() {
  ...
  for (gene in genes_f) {
    ...
    p = ggplot(data = df_f,aes(x = time,y = expression,color = mc,linetype = mc)) +
      geom_line(size = 1.5) + scale_color_manual(values = mc_color,labels = c("Crown cell","Notochord","Ciliary node","Foregut Bulk")) + 
      scale_linetype_manual(values = linetype_values,labels = c("Crown cell","Notochord","Ciliary node","Foregut Bulk"))+
      ggtitle(label = gene) + theme(legend.position = "none",plot.title = element_text(size = 20,hjust = 0.5)) + xlab("Time") + ylab("Expression")
  ...
  df_f = df_plot[df_plot$gene == gene,]
  p = ggplot(data = df_f,aes(x = time,y = expression,color = mc,linetype = mc)) +
    geom_line(size = 1.5) + scale_color_manual(values = mc_color,labels = c("Crown cell","Notochord","Ciliary node","Foregut Bulk")) + scale_linetype_manual(values = linetype_values,labels = c("Crown cell","Notochord","Ciliary node","Foregut Bulk"))+
    ggtitle(label = gene) + theme(legend.key.size = unit(1.5,"cm")) + labs(color = "Cell type",linetype = "Cell type") +
  ...
  p_all = arrangeGrob(plot_list[["Dnmt1"]],plot_list[["Uhrf1"]],
               ...
               plot_list[["Tet3"]],p_leg,ncol = 2,nrow = 3,
...
cell_cycle_analysis = function() {
  ...
  for(mc_f in mc_node) {
    ...
    p = ggplot(df_plot) + geom_area(aes(x = score,y = y,fill = type),alpha = 0.4) + scale_fill_manual(values = fill_colors) +
      geom_line(aes(x = score,y = y,color = type),size = 1.5) + scale_color_manual(values = fill_colors) + ylab("") + xlab( "M/S Phase Score") +
      ggtitle(sprintf("MC #%d",mc_f)) + theme(plot.title = element_text(size = 20,hjust = 0.5)) + ylim(0,3.2) + xlim(0,2) + theme(legend.position = "none")
