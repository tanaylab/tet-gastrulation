...
atlas_proj_on_wt10 <- function(mat_query, fn, feat_genes, selected_metacells_for_plot = NULL, cex_points = 0.5, w = 1000, h = 1000, plot_pdf = F, main_tag = "", cex.main = 1, plot_gray_background = T) {
    ...
    if (plot_gray_background) {
        plot(mc2d_wt@sc_x[all_cells], mc2d_wt@sc_y[all_cells],
            col = "gray90", pch = 19, cex = cex_points, xaxt = "n", yaxt = "n", xlab = "", ylab = "", axes = F, main = main_tag,
            cex.main = cex.main, ylim = ylim, xlim = xlim
        )
        points(ref_x, ref_y, pch = 19, col = mc_wt@colors[best_ref], cex = cex_points)
    } else {
        plot(
            x = ref_x, y = ref_y, pch = 19, col = mc_wt@colors[best_ref],
...
plot_wt_manifold <- function() {
    ...
    pdf(file = fn, width = w, height = h, useDingbats = F)
    plot(mc2d_wt@sc_x[names(mc_wt@mc)], mc2d_wt@sc_y[names(mc_wt@mc)],
        col = mc_wt@colors[mc_wt@mc], pch = 19, cex = cex_points, xaxt = "n", yaxt = "n", xlab = "", ylab = "", axes = F,
        ylim = ylim, xlim = xlim
    )
    ...
    pdf(file = fn, width = w, height = h, useDingbats = F)
    cex_points <- 1
    plot(mc2d_wt@mc_x, mc2d_wt@mc_y,
        col = mc_wt@colors, pch = 19, cex = cex_points, xaxt = "n", yaxt = "n", xlab = "", ylab = "", axes = F,
        ylim = ylim, xlim = xlim
    )
    ...
    segments(
        x0 = mc2d_wt@mc_x[segments_from_mc], y0 = mc2d_wt@mc_y[segments_from_mc],
