...
tko_chim_timing = function() {
  ...
  chim_emb_summary[rownames(time_dist_host_ko$host),"best_rank_host"] = time_dist_best_match(atlas_time_dist = time_dist_host_ko$atlas_time_dist,
                                                                                             query_time_dist = time_dist_host_ko$host)
  chim_emb_summary$best_rank_ko = NA
  chim_emb_summary[rownames(time_dist_host_ko$ko),"best_rank_ko"] = time_dist_best_match(atlas_time_dist = time_dist_host_ko$atlas_time_dist,
                                                                                         query_time_dist = time_dist_host_ko$ko)
  chim_emb_summary$best_rank_host_control = NA
  chim_emb_summary[rownames(time_dist_host_ko$host_control),"best_rank_host_control"] = time_dist_best_match(atlas_time_dist = time_dist_host_ko$atlas_time_dist,
                                                                                                   query_time_dist = time_dist_host_ko$host_control)
  
  chim_emb_summary$best_rank_control = NA
  chim_emb_summary[rownames(time_dist_host_ko$control),"best_rank_control"] = time_dist_best_match(atlas_time_dist = time_dist_host_ko$atlas_time_dist,
...
plot_cumulative_distribution_ko_host_per_chimera = function(mat_nm) {
  ...
  for (chimera in chim_embryos) {
    ...
    text(x = rep(x_pos_text,4), y = c(0.9,0.75,0.6,0.45),
         labels = c(sprintf("D = %1.1e",ks_ko_host_chim$statistic),sprintf("p < %1.1e",ks_ko_host_chim$p.value),
                    sprintf("N host = %d",N_host),sprintf("N KO = %d",N_ko)),cex = 1.2)
    dev.off()
...
plot_ko_vs_host_best_ranks = function(mat_nm) {
  ...
  plot(chim_emb_summary$best_rank_ko[f],chim_emb_summary$best_rank_host_control[f],pch = 19,xlim = c(rank_min,rank_max),ylim =  c(rank_min,rank_max),main = "KO vs host/control",
  ...
  plot(rank_to_time$developmental_time[chim_emb_summary$best_rank_ko[f]],
       rank_to_time$developmental_time[chim_emb_summary$best_rank_host_control[f]],pch = 19,
