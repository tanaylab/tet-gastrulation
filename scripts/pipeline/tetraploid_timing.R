...
tetra_timing = function(mat_nm) {
  ...
  time_dist_list = list(atlas_time_dist = atlas_time_dist$atlas_time_dist,
  ...
  chim_emb_summary[rownames(time_dist_list$query),"best_query"] = time_dist_best_match(atlas_time_dist = time_dist_list$atlas_time_dist,
...
tetraploid_plot_cor_time_dist_per_emb = function(mat_nm) {
  ...
  query_ref_cor = tgs_cor(t(as.matrix(as.data.frame.matrix(query_dist_all))),
  ...
  for (tetraploid in tetra_embryos) {
    ...
    plot(x = dev_time,y= query_ref_cor[tetraploid,],pch = 19,main = tetraploid,ylim = c(min(query_ref_cor),1),xlim =c(min(dev_time),max(dev_time)),
