...
dko12_gating <- function(plot_pdf = F) {
    mat_nm <- "dko12_chim"
    mat <- scdb_mat(mat_nm)
    ...
    plot(gfp_tr, fsc_a, pch = 19, cex = 0.5, xlab = "GFP", ylab = "FSC-A", col = cls_color, main = paste("All DKO12 chimera embryos", sep = " "), xlim = lim_gfp, ylim = lim_fsc)
    points(gfp_tr[!f], fsc_a[!f], cex = 0.5, pch = 19, col = cls_color[!f])
    dev.off()

    png(sprintf("%s/gfp_vs_ssc_all.png", fig_dir))
    plot(gfp_tr, ssc_a, pch = 19, cex = 0.5, xlab = "GFP", ylab = "SSC-A", col = cls_color, main = paste("All DKO12 chimera embryos", sep = " "), xlim = lim_gfp, ylim = lim_ssc)
    ...
    # KO clone - plates containing 0228 in the name are clone DKO12_10

    f_dko12_10 <- grep("0228", mat@cell_metadata[cls_ko, "plate"])

    ko_clone <- rep("DKO12_25", length(cls_ko))
    ko_clone[f_dko12_10] <- "DKO12_10"
    cls_clone[f_ko] <- ko_clone
    ...
    if (plot_pdf) {
        ...
        plot(gfp_tr, fsc_a, pch = 19, cex = 0.5, xlab = "GFP", ylab = "FSC-A", col = gate_color, main = paste("All DKO12 chimera embryos", sep = " "), xlim = lim_gfp, ylim = lim_fsc)
        abline(a = a_l, b = b_l, lty = "dashed")
        abline(a = a_h, b = b_h, lty = "dashed")
        legend(x = "topright", pch = 19, col = c("blue", "coral3", "gray70"), legend = c("Host", "DKO12", "unclear"))
        dev.off()
